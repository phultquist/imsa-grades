const courses = [
	`Modern World Fiction`,
	`Differential Equations`,
	`Biochemistry`,
	`Molecular and Cellular Biology`,
	`German III`,
	`Modern Theater`,
	`International Relations`,
	`Multi-Variable Calculus`,
	`Modern Physics`,
	`Physiology and Disease`,
	`# Student Inquiry and Research `,
	`BC Calculus II`,
	`Organic Chemistry I`,
	`Physics: Calculus-based Mechanics`,
	`Mandarin Chinese II`,
	`Creative Writing Workshop`,
	`BC Calculus III`,
	`Discrete Mathematics`,
	`Graphic Novels: Image and Text`,
	`History of Philosophy`,
	`AB Calculus I`,
	`Problem Solving`,
	`Evolution, Biodiversity, and Ecology`,
	`Ceramics`,
	`History of Technology and Culture`,
	`AB Calculus II`,
	`Object Oriented Programming`,
	`Organic Chemistry II`,
	`# Student Inquiry and Research/Summer`,
	`Topics in Recent United States History`,
	`Biophysics`,
	`Environmental History`,
	`Physics: Sound and Light`,
	`Seminar in Biology: Virology`,
	`German IV`,
	`Microeconomics`,
	`Advanced Programming`,
	`Gender Studies`,
	`Applied Engineering`,
	`Photography`,
	`Environmental Chemistry`,
	`Web Technologies I`,
	`French IV`,
	`Speculative Fiction Studies`,
	`Web Technologies II`,
	`Spanish IV`,
	`Advanced Ceramics`,
	`Tennis And Badminton`,
	`Physics: Calculus-based Elec & Magnetism`,
	`Planetary Science`,
	`Physics: Applied Mechanics`,
	`Spanish V`,
	`History of Biology`,
	`History of Astronomy`,
	`Mathematical Investigations IV`,
	`Russian I`,
	`Art Design`,
	`Lifeguarding and Water Polo`,
	`Mandarin Chinese III`,
	`Film Study: History and Criticism`,
	`Macroeconomics`,
	`Outdoor and Indoor Games`,
	`Polyhedra and Geometric Sculpture`,
	`Medieval Societies`,
	`The Idea of the Individual`,
	`Chamber Strings`,
	`20th Century Poetry`,
	`Microbes and Disease`,
	`Russian III`,
	`Movement and Relaxation`,
	`Adv. Chemistry--Structure and Properties`,
	`Introduction to Algebraic Structures I`,
	`Advanced Chemistry--Chemical Reactions`,
	`Political Theory`,
	`Japanese III`,
	`Theory of Analysis`,
	`The World in the Twentieth Century`,
	`Russian II`,
	`BC Calculus I`,
	`French V`,
	`Chamber Choir`,
	`Wind Ensemble`,
	`German V`,
	`Computational Thinking`,
	`Statistical Exploration and Description`,
	`Statistical Experimentation & Inference`,
	`French I`,
	`Japanese I`,
	`Number Theory`,
	`European History`,
	`French III`,
	`BC Calculus II/III`,
	`String Orchestra`,
	`Mathematical Investigations III`,
	`Mandarin Chinese I`,
	`Concert Choir`,
	`Japanese II`,
	`Spanish III`,
	`Concert Band`,
	`Literary Explorations III`,
	`Power and Authority in History`,
	`Robotics Programming`,
	`Engineering`,
	`Ancient World Religion and Philosophy`,
	`German II`,
	`Seminar in Biology: Stem Cell Biology`,
	`Victorian Fiction`,
	`Scientific Inquiries - Biology`,
	`Conflict in World History`,
	`Dance`,
	`Music Theory`,
	`US Government and the Constitution`,
	`Graph Theory with Applications`,
	`Scientific Inquiries - Chemistry`,
	`French II`,
	`Advanced Problem Solving`,
	`BC Calculus I/II`,
	`Geometry I/II`,
	`American Studies`,
	`German I`,
	`Moving and Learning`,
	`Literary Explorations I`,
	`Scientific Inquiries - Physics`,
	`Methods in Scientific Inquiry`,
	`Literary Explorations II`,
	`Mathematical Investigations II`,
	`The History of China and India`,
	`Seminar in Biology: Neurobiology`,
	`Mathematical Investigations I/II`,
	`Tolkien: Language and Literature`,
	`Computational Science`,
	`Spanish II`,
	`Survey of Organic Chemistry`,
	// `# Advanced Topics in Mathematics`,
	`Sci Inquiries - Molecular Genetics`,
	`Sci Inquiries - Organisms and Ecosystems`,
	`Seminar in Biology: Animal Behavior`,
	`History of Cultural Contact`,
	`Rise of the Atlantic World, 1492-1815`,
	`Seminar in Biology: Bioinformatics`,
	`Topics in Current Affairs`,
	`Modern Geometries`,
	`Printmaking`,
	`Shakespeare`,
	`Music Appreciation`,
	`Computer Science Inquiry`,
	`Art, Worldview, and Society in History`,
	`Web Technologies`,
	`Art and Design`,
	`Drawing and Illustration`,
	`CS Seminar: Advanced Web Technologies`,
	`Mathematical Modeling`,
	`Digital Photography`,
	`Scientific Illustration`,
	`Historic Global Commodities and Culture`,
	`African American Studies (History focus)`,
	`Modern Genocide and Mass Violence`,
	`Microcontroller Applications (CS)`,
	`CS Seminar: Security and Android Apps`,
	`Microcontroller Applications (Science)`,
	`Advanced Biological Systems`,
	`Seminar in Biology: Molecular Bio Lab`,
	`Net and Wall Games`,
	`Digital Literary Studies`,
	`Observational Drawing`,
	`America in the Contemporary World`,
	`Advanced Web Technologies`,
	`Rhetoric and Communication: Science`,
	`3D Design Foundations`,
	`Modern Economics`,
	`History of the Environment`,
	`CS Seminar: Machine Learning`,
	`Linear Algebra`,
	`Medicinal Chemistry`,
	`Geology`,
	`Cancer Biology`,
	`Pathophysiology`,
	`Biology of Behavior`,
	`Stress Management for Life`
];

export default courses;
